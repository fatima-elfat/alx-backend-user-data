...
class SessionAuth(Auth):
        ...
        creates a Session ID for a user_id:
            ...
            Otherwise:
                ...
                Use this Session ID as key of the dictionary
                    user_id_by_session_id - the value for
                    this key must be user_id.
                Return the Session ID.
            The same user_id can have multiple Session ID - indeed, the
    ...
    def user_id_for_session_id(self, session_id: str = None) -> str:
        ...
        You must use .get() built-in for accessing
    ...
    def current_user(self, request=None):
        ...
        You must use self.session_cookie(...)
            and self.user_id_for_session_id(...) to return
            the User ID based on the cookie _my_session_id.
        By using this User ID, you will be able to retrieve a User
    ...
    def destroy_session(self, request=None):
        ...
        deletes the user session / logout:
            ...
            If the request doesn’t contain the Session
                ID cookie, return False - you must
                use self.session_cookie(request)
            If the Session ID of the request is not linked
                to any User ID, return False - you must use
                self.user_id_for_session_id(...)
            Otherwise, delete in self.user_id_by_session_id
                the Session ID (as key of this dictionary)
                and return True
        """
        if request is None:
            return False
